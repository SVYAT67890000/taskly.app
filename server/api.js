...
function getProjectMembers(projectId) {
  const db = getDb();
  return db.prepare('SELECT user_id FROM project_members WHERE project_id = ?')
    .all(projectId)
    .map(r => r.user_id);
}

function userCanAccessProject(userId, projectId) {
  const db = getDb();
...
function filterFriendIds(db, userId, ids) {
  return Array.from(new Set((ids || []).filter(id => id !== userId && areFriends(db, userId, id))));
}

function fetchRelatedUsers(db, userId) {
  const ids = new Set([userId]);

  db.prepare('SELECT friend_id AS id FROM friendships WHERE user_id = ?').all(userId)
    .forEach(r => ids.add(r.id));
  db.prepare('SELECT from_user_id AS id FROM friend_requests WHERE to_user_id = ? AND status = ?')
    .all(userId, 'pending').forEach(r => ids.add(r.id));
  db.prepare('SELECT to_user_id AS id FROM friend_requests WHERE from_user_id = ? AND status = ?')
    .all(userId, 'pending').forEach(r => ids.add(r.id));
  db.prepare(`
  ...
  return db.prepare(`SELECT * FROM users WHERE id IN (${placeholders})`).all(...idList).map(formatUser);
}

function getFriendRequestsForUser(db, userId) {
...
router.post('/auth/reset-password', (req, res) => {
  ...
  const stored = db.prepare('SELECT code, expires_at FROM password_codes WHERE user_id = ?').get(row.id);
  if (!stored || Date.now() > stored.expires_at) {
    db.prepare('DELETE FROM password_codes WHERE user_id = ?').run(row.id);
...
router.get('/bootstrap', authMiddleware, (req, res) => {
  ...
  const projects = projectRows.map(p => rowToProject(p, getProjectMembers(p.id)));

  const friendIds = db.prepare(
    'SELECT friend_id FROM friendships WHERE user_id = ?'
  ).all(userId).map(r => r.friend_id);

  const { incoming, outgoing } = getFriendRequestsForUser(db, userId);

  const achievements = getUserAchievements(userId);
  const stats = getStats(userId);
  const users = fetchRelatedUsers(db, userId);

  const noteRows = db.prepare('SELECT * FROM notes WHERE user_id = ? ORDER BY pinned DESC, updated_at DESC')
    .all(userId);
...
router.post('/users/password', authMiddleware, (req, res) => {
  const bcrypt = require('bcryptjs');
  ...
  const db = getDb();
  const stored = db.prepare('SELECT code, expires_at FROM password_codes WHERE user_id = ?').get(req.user.id);
  if (!stored || Date.now() > stored.expires_at) {
    db.prepare('DELETE FROM password_codes WHERE user_id = ?').run(req.user.id);
    return res.status(400).json({ error: 'Код просрочен или не запрашивался' });
  }
...
router.patch('/support/tickets/:id', authMiddleware, (req, res) => {
  ...
  if (updates.length > 1) {
    db.prepare(`UPDATE support_tickets SET ${updates.join(', ')} WHERE id = @id`).run(params);
  }
...
router.post('/tasks', authMiddleware, (req, res) => {
  ...
  if (task.projectId && !userCanAccessProject(req.user.id, task.projectId)) {
...
router.delete('/tasks/:id', authMiddleware, (req, res) => {
  getDb().prepare('DELETE FROM tasks WHERE id = ? AND owner_id = ?').run(req.params.id, req.user.id);
  res.json({ ok: true });
});

router.post('/tasks/sync', authMiddleware, (req, res) => {
...
router.post('/projects', authMiddleware, (req, res) => {
  ...
  res.json({
    project: rowToProject(row, getProjectMembers(id)),
    newAchievements
  });
});
...
router.put('/projects/:id', authMiddleware, (req, res) => {
  const db = getDb();
  if (!userCanAccessProject(req.user.id, req.params.id)) {
    return res.status(403).json({ error: 'Нет доступа' });
  }
  const { name, description, memberIds } = req.body || {};
  if (name) {
  ...
  if (memberIds) {
    ...
    for (const uid of members) {
      insertMember.run(req.params.id, uid);
    }
  }
  const row = db.prepare('SELECT * FROM projects WHERE id = ?').get(req.params.id);
  const newAchievements = checkAndUnlock(req.user.id);
  res.json({
    project: rowToProject(row, getProjectMembers(req.params.id)),
    newAchievements
  });
});
...
router.post('/friends/request', authMiddleware, (req, res) => {
  ...
  const targetRow = findUserByTagOrPublicId(query);
...
router.post('/friends/respond', authMiddleware, (req, res) => {
  ...
  const newAchievements = checkAndUnlock(req.user.id);
  if (accept) checkAndUnlock(userId);
  res.json({ ok: true, newAchievements });
});

router.post('/friends/cancel', authMiddleware, (req, res) => {
...
router.get('/friends', authMiddleware, (req, res) => {
  ...
  const { incoming, outgoing } = getFriendRequestsForUser(db, req.user.id);
  const users = fetchRelatedUsers(db, req.user.id);
...
// ——— Messages ———
router.get('/messages/:friendId', authMiddleware, (req, res) => {
  const db = getDb();
...
router.post('/messages/:friendId', authMiddleware, (req, res) => {
  const db = getDb();
...
router.get('/messages/feed', authMiddleware, (req, res) => {
  const db = getDb();
...
router.get('/messages/inbox', authMiddleware, (req, res) => {
  const db = getDb();
...
router.post('/conversations', authMiddleware, (req, res) => {
  ...
  const friends = filterFriendIds(db, req.user.id, memberIds || []);
  if (!friends.length) {
  ...
  const id = generateId();
  const now = Date.now();
  db.prepare('INSERT INTO conversations (id, name, owner_id, created_at) VALUES (?, ?, ?, ?)')
...
router.get('/stats', authMiddleware, (req, res) => {
  ...
  const projectsProgress = db.prepare(`
    SELECT p.id, p.name,
...
// ——— Mind maps ———
router.get('/mindmaps/:projectId', authMiddleware, (req, res) => {
  if (!userCanAccessProject(req.user.id, req.params.projectId)) {
    return res.status(403).json({ error: 'Нет доступа' });
  }
  const db = getDb();
  let row = db.prepare('SELECT * FROM mind_maps WHERE project_id = ?').get(req.params.projectId);
  if (!row) {
    const now = new Date().toISOString();
    db.prepare(`
    ...
    row = db.prepare('SELECT * FROM mind_maps WHERE project_id = ?').get(req.params.projectId);
  }
  res.json({
...
router.put('/mindmaps/:projectId', authMiddleware, (req, res) => {
  if (!userCanAccessProject(req.user.id, req.params.projectId)) {
    return res.status(403).json({ error: 'Нет доступа' });
  }
...
router.post('/notes', authMiddleware, (req, res) => {
  ...
  const newAchievements = checkAndUnlock(req.user.id);
  res.json({ note: rowToNote(db.prepare('SELECT * FROM notes WHERE id = ?').get(id)), newAchievements });
});
...
router.post('/uploads', authMiddleware, (req, res) => {
...
router.get('/uploads/:userId/:fileName', authMiddleware, (req, res) => {
  if (req.user.id !== req.params.userId) {
    const isFriend = getDb().prepare(
      'SELECT 1 FROM friendships WHERE user_id = ? AND friend_id = ?'
...
router.get('/friends/:friendId/shared-tasks', authMiddleware, (req, res) => {
  const db = getDb();
...
router.get('/users/search', authMiddleware, (req, res) => {
  ...
  const row = findUserByTagOrPublicId(q);
